...
class APIService {
    private let baseURL = "http://3.209.80.245:3000"
    ...
    func getStudentSignatureUploadUrl(userId: String) async throws -> (uploadUrl: URL, key: String) {
        ...
        guard let url = URL(string: "\(baseURL)/users/\(userId)/student-signature-upload-url") else { throw APIError.invalidURL }
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
        request.setValue("Bearer \(token.jwt)", forHTTPHeaderField: "Authorization")
        request.setValue("application/json", forHTTPHeaderField: "Accept")
        
        logger.info("Requesting S3 upload URL for student signature, user id: \(userId)")
    ...
    func uploadStudentSignatureToS3(uploadUrl: URL, imageData: Data) async throws {
        ...
        logger.info("Uploading student signature data (\(imageData.count) bytes) directly to S3...")
        ...
        do {
            ...
            logger.info("Student signature successfully uploaded to S3.")
    ...
    func saveStudentSignatureReference(userId: String, signatureKey: String) async throws -> UserResponse {
        ...
        guard let token = try? await session.getToken() else { throw APIError.tokenUnavailable }
        // Note the change in endpoint - no submissionId needed
        guard let url = URL(string: "\(baseURL)/users/\(userId)/student-signature") else { throw APIError.invalidURL }
        ...
        logger.info("Saving student signature reference (key: \(signatureKey))")
        ...
        do {
            ...
            do {
                ...
                logger.info("Successfully saved student signature reference")
                return updatedUser
            } catch {
                logger.error("API Error: Failed to decode save reference response JSON - \(error)")
                throw APIError.decodingError(error)
    ...
    func getStudentSignatureViewUrl(userId: String) async throws -> URL? {
        ...
        logger.info("Requesting student signature view URL")
        ...
        do {
            ...
            guard (200...299).contains(httpResponse.statusCode) else {
                ...
                // Handle 404 specifically if backend sends it when signatureUrl is null
                    ...
                    logger.info("Student signature not found on backend.")
                    return nil // Return nil explicitly if backend indicates not found
                }
                else { throw APIError.serverError(statusCode: httpResponse.statusCode, message: responseBody) }
            }
