...
class HomeViewModel: ObservableObject {
    ...
    @Published var submittedHours: Double = 0
    @Published var approvedHours: Double = 0
    ...
    func fetchUserSubmissions() async {
        do {
            ...
            self.completeSubmissions = submissions.filter { $0.status == "SUBMITTED" || $0.status == "APPROVED" }
    ...
    func updateHours() {
        ...
        submittedHours = submissions.filter { $0.status == "SUBMITTED" }.reduce(0) { total, submission in
            total + (submission.hours ?? 0)
        }
        
        // Calculate approved hours (from all submissions with APPROVED status)
        approvedHours = submissions.filter { $0.status == "APPROVED" }.reduce(0) { total, submission in
            total + (submission.hours ?? 0)
        }
